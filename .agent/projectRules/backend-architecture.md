...
- **Password hash bằng `bcryptjs` (không phải `bcrypt` native).** `bcrypt` cần `node-gyp`/MSVC build tools — máy dev Windows không có sẵn thường sẽ fail cài đặt. `bcryptjs` thuần JS, cùng API, chậm hơn không đáng kể ở cost factor 12 cho quy mô skeleton này.
- **`AuthService.login()` chạy dummy `bcryptjs.compare()` khi email không tồn tại**, trước khi throw `UnauthorizedException`. Đây là timing-attack mitigation — nếu bỏ nhánh dummy compare, response time của "email không tồn tại" (nhanh) khác "sai password" (chậm, chờ bcrypt), lộ ra email nào tồn tại trong hệ thống qua đo thời gian phản hồi. Message lỗi PHẢI generic (`'Invalid email or password'`) cho cả 2 trường hợp.
- **`env.schema.ts` throw lỗi boot nếu `NODE_ENV=production` và `JWT_SECRET` vẫn là giá trị default `'dev-placeholder-secret'`.** Giá trị default này nằm trong source code (public), chỉ an toàn cho dev/skeleton — nếu vẫn dùng nó ở production, bất kỳ ai đọc được source đều tự ký được JWT hợp lệ cho user id bất kỳ, bypass toàn bộ guard.
